...
A minimal, fully-visible Retrieval-Augmented Generation pipeline over the VWO Product
Requirements Document. The React UI shows every stage of the pipeline as it happens:
PDF ingestion → chunking → embedding (Nomic) → storage (ChromaDB) → retrieval → answer
generation (Groq).

## Architecture

- **Backend** (`backend/`): FastAPI app. On startup it automatically reads the PDF from
  `data/`, splits it into overlapping chunks, embeds each chunk locally with the
  `nomic-ai/nomic-embed-text-v1.5` model (via `sentence-transformers`, no API key needed),
  and stores the vectors in a local persistent ChromaDB collection
  (`backend/chroma_db/`). Queries embed the question, retrieve the top-4 most similar
  chunks from ChromaDB, and send them as context to Groq's `openai/gpt-oss-120b` model to
  generate the final answer.
- **Frontend** (`frontend/`): React + Vite app that polls ingestion status, renders the
  pipeline stepper and an Ingestion panel (source folder, stats, sample embedding, chunk
  preview, plus manual **Ingest PDF** / **Reset** controls), and provides the query UI.
  Talks to the backend at `/api/*` (proxied to `http://localhost:8000` in dev).
...
1. **Read PDF** — `pypdf` extracts text per page from the source PDF.
...
| `/api/ingest`          | POST   | Force a fresh ingestion run (used by the "Ingest PDF" button). |
