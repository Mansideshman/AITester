...
export function KanbanBoard({ jobs, setJobs, onUpdateJob, onEdit, onDelete, onSelectJob, onApply }: KanbanBoardProps) {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (activeId !== overId) {
      setJobs((jobs) => {
        const activeIndex = jobs.findIndex((j) => j.id === activeId);
        const overIndex = jobs.findIndex((j) => j.id === overId);
        // overId may be a column id (dropped on empty column space) rather
        // than a job id — in that case there's nothing to reorder against.
        if (activeIndex === -1 || overIndex === -1) return jobs;
        return arrayMove(jobs, activeIndex, overIndex);
      });
    }
  ...
  return (
    ...
    >
      ...
      <DragOverlay dropAnimation={{ sideEffects: defaultDropAnimationSideEffects({ styles: { active: { opacity: '0.5' } } }) }}>
        {activeJob ? (
          <JobCard job={activeJob} onEdit={onEdit} onDelete={onDelete} onApply={onApply} />
        ) : null}
      </DragOverlay>
    </DndContext>
  );
