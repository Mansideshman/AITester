...
export function KanbanColumn({ status, jobs, onEdit, onDelete, onSelectJob, onApply }: KanbanColumnProps) {
      ...
      >
        <SortableContext items={jobs.map((j) => j.id)} strategy={verticalListSortingStrategy}>
          {jobs.map((job) => (
            ...
            <JobCard job={job} onEdit={onEdit} onDelete={onDelete} onApply={onApply} />
          </div>
          ))}
        </SortableContext>
      </div>
